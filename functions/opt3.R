...
opt3 <- function(x,lbd_jp1, tau_cand, s,lbd_mjp_c,y, Dmatrix, DmatrixZ, DmatrixZd){
  ...
  # Calculate the inside of exponential part with continuous Z only
  exp.partz = tensor::tensor(DmatrixZ,lbd_gamma[1:Q+P]^2,3,1)
  # Calculate the inside of exponential part with discrete Z only
  exp.partzd = tensor::tensor(DmatrixZd,lbd_gamma[1:R+(Q+P)]^2,3,1)
