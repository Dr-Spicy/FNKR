...
for (repetationnumber in 1:repeatlimit){
  ...
  FPCAsparse <- foreach(diter = 1:P, .packages = 'fdapace') %dopar% {
    zs = FPCA(xSparse_all[[diter]]$xNoisy, xSparse_all[[diter]]$Lt,
  ...
  X1 = fitted(FPCAsparse[[1]])
  X2 = fitted(FPCAsparse[[2]])
  X3 = fitted(FPCAsparse[[3]])
  X4 = fitted(FPCAsparse[[4]])
  X5 = fitted(FPCAsparse[[5]])
...
selection.res = t(apply(matres,2,mean))*repeatlimit


save(ARMSE, file = '~/Functional Kernel Regression/FAM-comparison/sparse/me40sn4/ARMSE.Rdata')
save(selection.res, file = '~/Functional Kernel Regression/FAM-comparison/sparse/me40sn4/selection.res.Rdata')
