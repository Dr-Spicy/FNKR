...
num_m = sample(x=matrix(rep(c(5,6,7),N),nrow= N), size = N)
...
temp = sapply(xNoisy_all[[diter]],exp)
...
for (i in 1:N){
  xTrue[[i]] = Ksi[i,] %*% t(matrix(c(eigFunct4(sampled_t[[i]]),
                                      eigFunct5(sampled_t[[i]]),eigFunct6(sampled_t[[i]])), 
...
# m3 is cubic then find the average
# apply the defined fucntion on this X
temp = sapply(xNoisy_all[[diter]],cubic)
# convert list to matrix with fillers = 0
temp = list.as.matrix(temp, byrow = T, filler = 0)
# get the mean over row for each sample
temp1  = apply(temp,1,mean)
...
# m4 is cubic then find the average
# apply the defined fucntion on this X
temp = sapply(xNoisy_all[[diter]],cubic)
# convert list to matrix with fillers = 0
temp = list.as.matrix(temp, byrow = T, filler = 0)
...
pi_cand = seq(0, 1e-1, length.out = 11)
  ...
  repeat{
    for (jp in 1:P){
      ...
      res <- nloptr(x0 = 0,
                    eval_f = opt,
                    lb = 0, ub = pi_cand[s],
                    opts = list("algorithm" = 'NLOPT_GN_CRS2_LM', 'print_level' = 0,
                                "xtol_rel" = 1e-7),
