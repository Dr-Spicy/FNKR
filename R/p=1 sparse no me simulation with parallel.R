...
FPCAsparse <- FPCA(training$x, training$t,
                  list(plot = T, FVEthreshold = 0.95, nRegGrid = 501, verbose = T, error = F))
...
#### save the objects ####
save(FPCAsparse, file = '1x_sparse_noerr_FPCA.Rdata')
#FPCAdense <- readRDS('1x_dense_FPCA.Rdata')

save(pred_zeta, file = '1x_sparse_noerr_pred_zeta.RData')
#load('1x_dense_pred_zeta.RData')
save(Dtbt, file = '1x_sparse_noerr_Dtbt.RData')
...
which(RMSE == min(na.omit(RMSE))) # 17

# define the bandwidth
h = seq(h[16], h[18], by = 0.0001)
