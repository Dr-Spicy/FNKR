...
N <- 500;  # when N=500, PACE time = 657s
P <- 1;
M <- 501;
set.seed(100)
...
#### save the objects ####
#save(FPCAsparse, file = '1x_sparse_me_FPCA.Rdata')
load('1x_sparse_me_FPCA.Rdata')

#save(pred_zeta, file = '1x_sparse_me_pred_zeta.RData')
load('1x_sparse_me_pred_zeta.RData')
#save(Dtbt, file = '1x_sparse_me_Dtbt.RData')
load('1x_sparse_me_Dtbt.RData')
...
plot(y_hat0, col = 'red', main = 'p=1 sparse me = .2 simulation', ylim = c(0, 50))
lines(yTrue[451:500], col ='black')

## Calculate the coefficient of determinant 
SSTO = sum((yTrue[451:500] - mean(yTrue[451:500]))^2)
SSRes = sum((y_hat0 - yTrue[451:500])^2)
R2 = 1 - SSRes/SSTO  # 0.861


#### side note ####

# solve funtion is screwing things up in the GetCEScores function used in FPCA function
