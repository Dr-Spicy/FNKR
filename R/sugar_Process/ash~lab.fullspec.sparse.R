...
##################################################################################################################################
# Do not use Proc variables for now
Proc <- data$Proc
colSums(is.na(Proc))
dim(Proc) <- data$DimProc
##################################################################################################################################
# non-functional Z
Lab <- data$Lab
colSums(is.na(Lab))
dim(Lab) <- data$DimLab
# Only use the first 48 columns for the rest Lag are almost all NA
Lab.use <- Lab[,,1]
# give Lab.use col names
colnames(Lab.use) = paste('Lab',data$LabNumber, sep = '_')
# Find the col (Lab.variables) with less than 10 NaN, have 23 variables left
Lab.use = Lab.use[,colSums(is.na(Lab.use)) < 10]
# Extract the samples with no NaN in Lab measurements
Use.index = rowSums(is.na(Lab.use)) ==0
# Remove samples with NaNs
Lab.use = Lab.use[Use.index,]
...
if (method.for.Dist == 'D1'){
    ...
    tau_cand <- c( seq(4,6, 0.1))} # tau_cand <- c(seq(7.6,8.4,0.2), seq(8.6,10, 0.1), seq(10.2, 11, .2))}
