...
if (full.spec == 'Yes'){
    ...
    fluo.truc = fluo[1:268,131:571,1:7]
}
...
Lab.use.imp = mice(Lab.use, method = 'midastouch', maxit = 10,  seed = 127)
##################################################################################################################################

#### Define the simulation parameters ####

# Set the number of subjects (N) and the size of train, tune, test
N <- 268; n1 <- .5*N; n2 <- .25*N; n3 <- .25*N;
...
if (method.for.Dist == 'D1'){
    ...
    tau_cand <- c( seq(4, 6.5, 0.1))} # tau_cand <- c(seq(7.6,8.4,0.2), seq(8.6,10, 0.1), seq(10.2, 11, .2))}
...
for (kiter in min.K.used:max.K.used){
    ...
    condition1 = identical(which(opted_lbd[kiter,] > 0), relavent.indecies)
    
    # check if all irrelavent predictors are excluded
    
    condition2 = identical(which(opted_lbd[kiter,] == 0), irrelavent.indecies)
